...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i in arr) {
    answerArr.push(Math.pow(2, arr[i]));
  }
  return answerArr;
};
...
const forEachTwoToThe = (arr) => {
  ...
  arr.forEach( data => {
    answerArr.push(Math.pow(2, data));
  });
  return answerArr;
};
...
const mapTwoToThe = (arr) => {
  // Solution code here...
  return arr.map(data => Math.pow(2, data));
};
...
const charCode = (arr) => {
  // Solution code here...
  let characters = arr.map(data => data.charCodeAt());
  return characters;
};
...
const extractStats = (arr) => {
  // Solution code here...
  return arr.map(data => {
    return {name: data.stat.name, total: (data.baseStat + data.effort)};
  });
};
